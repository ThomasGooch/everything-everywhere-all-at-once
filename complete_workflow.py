...
async def complete_workflow(task_id: str, branch_name: str):
    ...
    try:
        ...
        try:
            ...
            # Move to Done
            target_status = "Done" if "Done" in transition_names else transition_names[0]
            result = await jira_api.transition_issue_async(task_id, target_status)
            
            if result.get('success', False):
                print(f"   ✅ Task moved to {target_status}!")
            else:
                print(f"   ⚠️ Failed to move task: {result.get('error', 'Unknown error')}")
...
The automated development workflow has completed successfully."""
            ...
            if result.get('success', False):
                ...
                print(f"   ⚠️ Failed to add comment: {result.get('error', 'Unknown error')}")
...
async def main():
    ...
    if len(sys.argv) != 3:
        ...
        print("Example: python complete_workflow.py CMMAI-49 CMMAI-49_create_initial_project_setup_521301")
        sys.exit(1)
