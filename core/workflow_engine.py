...
from core.variable_resolver import VariableResolver, VariableResolutionError
...
class WorkflowEngine:
    ...
    def _validate_dependencies(self, workflow: Workflow, errors: List[str], warnings: List[str]):
                ...
                if isinstance(input_value, str) and '${' in input_value:
                    ...
                    for ref in refs:
                        if '.' in ref:
                            ...
                            elif step_ref not in ['task', 'project', 'system', 'workflow']:
                                errors.append(f"Step '{step.name}' references undefined step: {step_ref}")
        
        # Detect circular dependencies using depth-first search
        self._detect_circular_dependencies(dependencies, errors)
    
    def _detect_circular_dependencies(self, dependencies: Dict[str, set], errors: List[str]):
        """Detect circular dependencies using depth-first search"""
        visited = set()
    ...
    def _validate_plugins(self, workflow: Workflow, errors: List[str], warnings: List[str]):
        ...
        for step in workflow.steps:
            if step.type == "plugin_action" and step.plugin:
                try:
                    ...
                    if not plugin:
                        errors.append(f"Plugin '{step.plugin}' not found for step '{step.name}'")
                    elif not hasattr(plugin, step.action):
                        warnings.append(f"Plugin '{step.plugin}' may not support action '{step.action}' for step '{step.name}'")
                except Exception as e:
    ...
    async def execute_workflow(self, workflow: Workflow, context: Dict[str, Any] = None) -> WorkflowResult:
        ...
        try:
            ...
            for step in workflow.steps:
                ...
                for output_name, context_key in step.outputs.items():
                    if output_name in step_result.outputs:
                        execution_context[context_key] = step_result.outputs[output_name]
                ...
                if not step_result.success:
                    ...
                    if error_strategy == ErrorStrategy.FAIL:
                        ...
                        logger.warning(f"Step '{step.name}' failed but continuing due to error strategy")
                        continue
                    # RETRY and ROLLBACK are handled within _execute_step
            
            # Check success criteria
            success = await self._check_success_criteria(workflow, execution_context, step_results)
            ...
            logger.info(f"Workflow execution completed: {workflow.name} (success={success})")
            return result
        ...
        for prerequisite in workflow.prerequisites:
            ...
            if condition:
                ...
                if not self._evaluate_condition(condition, context):
                    error_message = prerequisite.get('error_message', f"Prerequisite failed: {condition}")
                    raise WorkflowExecutionError(error_message)
    
    async def _should_execute_step(self, step: WorkflowStep, context: Dict[str, Any]) -> bool:
        ...
        for attempt in range(step.retry_count + 1):
            try:
                ...
                if attempt < step.retry_count:
                    logger.warning(f"{error_msg}, retrying (attempt {attempt + 1}/{step.retry_count + 1})")
    ...
    async def _execute_plugin_action(self, step: WorkflowStep, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not action_method:
            raise WorkflowExecutionError(f"Plugin '{step.plugin}' does not have action '{step.action}'")
        ...
        try:
            ...
            raise asyncio.TimeoutError(f"Plugin action timed out after {step.timeout} seconds")
    
    async def _execute_ai_action(self, step: WorkflowStep, context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _execute_system_action(self, step: WorkflowStep, context: Dict[str, Any]) -> Dict[str, Any]:
        """Execute a system action step"""
        # TODO: Implement system action execution
        # This includes logging, metrics collection, etc.
        logger.info(f"System action execution not yet implemented for step: {step.name}")
        return {"result": "system_action_placeholder"}
    
    async def _check_success_criteria(self, workflow: Workflow, context: Dict[str, Any], step_results: List[StepResult]) -> bool:
