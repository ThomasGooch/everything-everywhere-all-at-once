...
class VariableResolver:
    ...
    def resolve(
        self, template_string: str, context: Dict[str, Any], enable_cache: bool = True
    ) -> str:
        """
    ...
    def _resolve_recursive_variables(
        self, context: Dict[str, Any], max_depth: int = 5
    ) -> Dict[str, Any]:
        ...
        while changes_made and depth < max_depth:
            ...
            if current_state in previous_states:
                ...
                raise VariableResolutionError(
                    f"Circular reference detected in variables: {circular_vars}"
                )
        ...
        if depth >= max_depth:
            unresolved = [
                k for k, v in resolved.items() if isinstance(v, str) and "${" in v
            ]
            if unresolved:
                raise VariableResolutionError(
                    f"Maximum recursion depth exceeded, possible circular "
                    f"reference in: {unresolved}"
                )
        ...
        for key, value in resolved.items():
            if isinstance(value, str) and f"${{{key}}}" in value:
                raise VariableResolutionError(
                    f"Circular reference detected in variable: {key}"
                )
