...
class WorkspaceManager:
    ...
    async def create_workspace(
        self, agent_id: str, repository_url: str, setup_git: bool = False
    ) -> Workspace:
    ...
    async def cleanup_workspace(
        self, agent_id: str, preserve_logs: bool = True
    ) -> bool:
        ...
        try:
            ...
            raise WorkspaceCleanupError(
                f"Failed to cleanup workspace for {agent_id}: {e}"
            )

    async def cleanup_all_workspaces(
        self, preserve_logs: bool = True
    ) -> Dict[str, bool]:
