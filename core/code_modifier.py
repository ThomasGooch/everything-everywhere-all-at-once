...
class CodeModifier:
    ...
    SECURITY_PATTERNS = {
        "python": [
            ...
            (
                r'open\s*\([^)]*[\'"][wax]',
                "File write operation detected - review permissions",
            ),
        ],
        "javascript": [
            (r"eval\s*\(", "Use of eval() detected - security risk"),
        ...
        try:
            ...
            if validation_errors:
                raise ValidationError(
                    f"Code validation failed: {'; '.join(validation_errors)}"
                )
            ...
            try:
                for file_change in implementation.files:
                    ...
                    if success:
                        ...
                        raise ModificationError(
                            f"Failed to apply change to {file_change.path}"
                        )
                ...
                if backup_id:
                    try:
                        ...
                        logger.error(
                            f"Failed to restore backup during rollback: {restore_error}"
                        )
                ...
                for file_path in created_files:
                    try:
                        ...
                        logger.error(
                            f"Failed to cleanup {file_path} during rollback: {cleanup_error}"
                        )
    ...
    async def validate_generated_code(
        ...
        try:
            ...
            result.is_valid = (
                len(result.syntax_errors) == 0 and len(result.security_issues) == 0
            )

            logger.debug(
                f"Code validation result: valid={result.is_valid}, "
    ...
    async def restore_backup(self, workspace, backup_id: str) -> BackupResult:
        ...
        try:
            ...
            return BackupResult(
                success=True, backup_id=backup_id, files_restored=restored_count
            )
    ...
    async def _apply_file_change(self, workspace, file_change: FileChange) -> bool:
        ...
        try:
            if file_change.action == "create":
                ...
                if not file_path.exists():
                    ...
                    if not os.access(file_path, os.W_OK):
                        raise ModificationError(
                            f"No write permission for {file_change.path}"
                        )
            ...
            raise ModificationError(
                f"Failed to apply change to {file_change.path}: {e}"
            )
    ...
    async def _validate_security_issues(
        self, code: str, language: str, result: ValidationResult
    ):
