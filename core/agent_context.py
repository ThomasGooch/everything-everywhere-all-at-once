...
from .plugin_interface import PluginType, BasePlugin, PluginError
...
class AgentContext:
    ...
    async def initialize(self, plugin_paths: Optional[List[Path]] = None) -> bool:
        ...
        try:
            ...
            failed_plugins = [pid for pid, success in plugin_init_results.items() if not success]
            
            if failed_plugins:
                logger.error(f"Failed to initialize plugins: {failed_plugins}")
                # Continue with partial initialization - some plugins may not be critical
    ...
    async def cleanup(self) -> bool:
        ...
        try:
            ...
            if self._plugins_initialized:
                plugin_cleanup_results = await self.plugin_registry.cleanup_all_plugins()
                failed_cleanups = [pid for pid, success in plugin_cleanup_results.items() if not success]
    ...
    async def get_plugin(self, plugin_type: PluginType, plugin_name: str) -> Optional[BasePlugin]:
    ...
    async def create_plugin_instance(self, plugin_type: PluginType, plugin_name: str, 
        ...
        try:
            ...
            if not success:
                raise PluginError(f"Failed to initialize plugin {plugin_type.value}.{plugin_name}")
            
            logger.info(f"Created and initialized plugin: {plugin_type.value}.{plugin_name}")
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._plugins_initialized:
                ...
                unhealthy_plugins = [pid for pid, status in plugin_health.items() 
            ...
            # Add component-specific health checks here as they are implemented
    ...
    async def _create_configured_plugins(self):
        ...
        for plugin_name, config in plugin_configs.items():
            ...
            try:
                ...
                # Create plugin instance (but don't initialize yet - that happens in initialize_all_plugins)
                self.plugin_registry.create_plugin_instance(plugin_type, plugin_name, config)
                logger.info(f"Created configured plugin: {plugin_type_str}.{plugin_name}")
                
            except ValueError:
                logger.error(f"Unknown plugin type '{plugin_type_str}' for plugin {plugin_name}")
            except Exception as e:
