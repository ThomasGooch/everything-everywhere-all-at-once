...
class PluginRegistry:
    ...
    def register_plugin(
        ...
        if not hasattr(plugin_class, "get_plugin_type") or not hasattr(
            plugin_class, "get_plugin_name"
        ):
            raise PluginValidationError(
                f"Plugin class {plugin_class} must implement required methods"
            )
        ...
        if plugin_name in self._plugins[plugin_type_str]:
            logger.warning(
                f"Plugin {plugin_type_str}.{plugin_name} already registered, overwriting"
            )
    ...
    def create_plugin_instance(
        self, plugin_type: PluginType, plugin_name: str, config: Dict[str, Any]
    ) -> BasePlugin:
        ...
        try:
            ...
            if not instance.validate_config():
                raise PluginValidationError(
                    f"Invalid configuration for plugin {plugin_type.value}.{plugin_name}"
                )
            ...
            logger.error(
                f"Failed to create plugin instance {plugin_type.value}.{plugin_name}: {e}"
            )
    ...
    def list_plugins(
        ...
        if plugin_type:
            plugin_type_str = plugin_type.value
            return {
                plugin_type_str: list(self._plugins.get(plugin_type_str, {}).keys())
            }
    ...
    def discover_plugins(self, plugin_path: Path) -> int:
        ...
        if not plugin_path.exists() or not plugin_path.is_dir():
            logger.warning(
                f"Plugin path {plugin_path} does not exist or is not a directory"
            )
            return 0
                    ...
                    for name, obj in inspect.getmembers(module):
                        if (
                            ...
                            try:
                                ...
                                logger.error(
                                    f"Failed to register discovered plugin {name}: {e}"
                                )
    ...
    async def health_check_all_plugins(self) -> Dict[str, str]:
        ...
        for plugin_id, plugin_instance in self._instances.items():
            try:
                ...
                logger.error(
                    f"Exception during health check for plugin {plugin_id}: {e}"
                )
        ...
        for ptype_str, plugins in self._plugins.items():
            ...
            for plugin_name in plugins.keys():
                ...
                if plugin_id in self._instances:
                    ...
                    info["plugins_by_type"][ptype_str]["instances"].append(
                        instance_info
                    )
