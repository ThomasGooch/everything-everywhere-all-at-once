...
class CodebaseScanner:
    ...
    async def analyze_repository(self, repository_path: str) -> CodebaseAnalysis:
        ...
        if not repo_path.is_dir():
            raise BaseSystemError(f"Repository path is not a directory: {repository_path}")
        ...
        tech_stack = results[0] if not isinstance(results[0], Exception) else TechStack()
        architecture = results[1] if not isinstance(results[1], Exception) else ArchitecturePattern("unknown", 0.0)
        ...
        test_framework = results[5] if not isinstance(results[5], Exception) else TestFrameworkInfo()
        docs = results[6] if not isinstance(results[6], Exception) else DocumentationStyle()
    ...
    async def identify_architecture(self, repository_path: str) -> ArchitecturePattern:
        ...
        has_compose = any((repo_path / name).exists() for name in ['docker-compose.yml', 'docker-compose.yaml'])
    ...
    async def analyze_docs(self, repository_path: str) -> DocumentationStyle:
        ...
        if any((repo_path / name).exists() for name in ['CHANGELOG.md', 'CHANGELOG.rst', 'CHANGELOG.txt']):
            structure['changelog'] = True
        if any((repo_path / name).exists() for name in ['LICENSE', 'LICENSE.md', 'LICENSE.txt']):
    ...
    async def identify_configuration_files(self, repository_path: str) -> List[Dict[str, Any]]:
    ...
    def _is_configuration_file(self, filename: str) -> bool:
        ...
        return filename in self.CONFIG_FILES or filename.startswith('.') and len(filename) > 1
    ...
    async def _analyze_package_json(self, repo_path: Path, frameworks: set, libraries: set):
    ...
    async def _analyze_requirements_txt(self, repo_path: Path, frameworks: set, libraries: set):
    ...
    async def _analyze_pyproject_toml(self, repo_path: Path, frameworks: set, libraries: set):
    ...
    async def _has_database_config(self, repo_path: Path) -> bool:
        ...
        db_indicators = [
            'DATABASE_URL', 'DB_HOST', 'MONGO_URI', 'REDIS_URL',
            'database.yml', 'db.properties', 'hibernate.cfg.xml'
        ]
    ...
    async def _analyze_python_style(self, python_files: List[Path], style_analysis: Dict[str, Any]):
    ...
    async def _analyze_javascript_style(self, js_files: List[Path], style_analysis: Dict[str, Any]):
    ...
    async def _extract_python_patterns(self, python_files: List[Path]) -> List[CodePattern]:
    ...
    async def _extract_javascript_patterns(self, js_files: List[Path]) -> List[CodePattern]:
    ...
    async def _parse_package_json_deps(self, package_json_path: Path) -> List[DependencyInfo]:
    ...
    async def _parse_requirements_txt(self, requirements_path: Path) -> List[DependencyInfo]:
    ...
    async def _parse_pyproject_toml_deps(self, pyproject_path: Path) -> List[DependencyInfo]:
    ...
    async def _analyze_python_imports(self, python_files: List[Path]) -> Dict[str, Any]:
        ...
        if imports:
            ...
            common_patterns = [f"{module} (used {count} times)" for module, count in sorted_imports[:10]]
