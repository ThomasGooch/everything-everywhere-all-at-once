...
class TestGitHubPluginEnhanced:
    ...
    async def test_setup_workspace_with_analysis(self, enhanced_github_plugin):
        ...
        with patch.object(enhanced_github_plugin, 'clone_repository') as mock_clone, \
             patch.object(enhanced_github_plugin, 'create_branch') as mock_branch, \
             patch.object(enhanced_github_plugin, '_analyze_codebase_structure') as mock_analyze:
            
            mock_clone.return_value = PluginResult(success=True, data={"path": "/tmp/repo"})
            mock_branch.return_value = PluginResult(success=True, data={"branch": "feature/TEST-123-implement-feature"})
            ...
            result = await enhanced_github_plugin.setup_workspace_with_analysis(workspace_config)
            ...
            assert result.data["codebase_analysis"]["languages"] == ["Python", "JavaScript"]
    ...
    async def test_get_auto_reviewers(self, enhanced_github_plugin):
        ...
        large_file_changes = {
            "modified": [f"src/file_{i}.py" for i in range(600)],  # Exceeds size threshold of 500
            "created": [],
            "deleted": []
        }
    ...
    def test_render_pr_template(self, enhanced_github_plugin):
        ...
        pr_body = enhanced_github_plugin._render_pr_template("ai_generated", template_data)
