...
class TestVariableResolver:
    ...
    def setup_method(self):
        ...
        self.context = {
            ...
            "user_defined": {"environment": "staging", "timeout": 300},
        }
    ...
    def test_multiple_variable_substitution(self):
        ...
        template = (
            "${task.type}: ${task.title} (${task.id}) - Priority: ${task.priority}"
        )
        result = self.resolver.resolve(template, self.context)
    ...
    def test_circular_reference_detection(self):
        ...
        context = {"var_a": "${var_b}", "var_b": "${var_a}"}
...
class TestBuiltinTemplateFunctions:
    ...
    def test_string_functions(self):
        ...
        # Test replace
        assert (
            self.resolver.resolve("${text.replace(' ', '_')}", context) == "Hello_World"
        )
