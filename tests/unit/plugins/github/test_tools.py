...
from plugins.github.tools import GitHubTools, setup_workspace, complete_github_workflow, complete_github_workflow_async
...
class TestGitHubTools:
    ...
    def test_setup_repository_workspace_success(self, mock_github_api):
        ...
        mock_api = MagicMock()
        mock_api.clone_repository.return_value = {"success": True, "repo_url": "test_url"}
        mock_api.generate_branch_name.return_value = "TASK-123_implementation_123456"
        mock_api.create_branch.return_value = {"success": True, "branch": "TASK-123_implementation_123456"}
        mock_github_api.return_value = mock_api
        
        tools = GitHubTools()
        target_dir = Path("/tmp/test")
        
        result = tools.setup_repository_workspace(target_dir, "TASK-123", "implementation")
        ...
        mock_api.generate_branch_name.assert_called_once_with("TASK-123", "implementation")
        mock_api.create_branch.assert_called_once_with(target_dir, "TASK-123_implementation_123456")

    @patch('plugins.github.tools.GitHubAPI')
    def test_setup_repository_workspace_clone_failure(self, mock_github_api):
        """Test workspace setup when clone fails."""
        mock_api = MagicMock()
        mock_api.clone_repository.return_value = {"success": False, "error": "Clone failed"}
        mock_github_api.return_value = mock_api
    ...
    def test_setup_repository_workspace_branch_failure(self, mock_github_api):
        ...
        mock_api.create_branch.return_value = {"success": False, "error": "Branch creation failed"}
        mock_github_api.return_value = mock_api
    ...
    async def test_complete_workflow_async_success(self, mock_github_api):
        ...
        mock_api.create_pull_request_async = AsyncMock(return_value={
            "success": True, 
            "pr_number": 123, 
    ...
    async def test_complete_workflow_async_commit_failure(self, mock_github_api):
        ...
        mock_api = MagicMock()
        mock_api.generate_commit_message.return_value = "TASK-123: Test commit"
        mock_api.commit_changes.return_value = {"success": False, "error": "Commit failed"}
        mock_github_api.return_value = mock_api
    ...
    def test_check_workspace_changes_with_changes(self, mock_subprocess, mock_github_api):
