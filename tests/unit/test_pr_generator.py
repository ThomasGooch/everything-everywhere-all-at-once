...
from core.code_modifier import Implementation, FileChange
...
class TestPRGenerator:
    ...
    @pytest.mark.asyncio
    async def test_generate_pr_description_success(self, pr_generator, sample_task, sample_implementation, sample_test_results):
    ...
    @pytest.mark.asyncio
    async def test_generate_pr_description_with_breaking_changes(self, pr_generator, sample_task, sample_test_results):
    ...
    @pytest.mark.asyncio
    async def test_generate_review_checklist_feature(self, pr_generator, sample_implementation):
        ...
        assert "authentication" in security_items_text.lower() or "security" in security_items_text.lower()
    
    @pytest.mark.asyncio
    async def test_generate_review_checklist_bugfix(self, pr_generator, sample_implementation):
        ...
        assert "regression" in testing_text.lower() or "edge case" in testing_text.lower()
    ...
    @pytest.mark.asyncio
    async def test_format_pr_markdown(self, pr_generator, sample_task, sample_implementation, sample_test_results):
    ...
    async def test_detect_change_type(self, pr_generator):
        ...
        feature_files = [
            FileChange(path="src/new_feature.py", action="create", content="def new_function()"),
            FileChange(path="docs/feature.md", action="create", content="# New feature")
        ]
        change_type = await pr_generator._detect_change_type(feature_files, "Add new user management")
        assert change_type == "feat"
        
        # Test bug fix detection
        bug_files = [
            FileChange(path="src/bugfix.py", action="modify", content="# Fixed the issue")
        ]
        change_type = await pr_generator._detect_change_type(bug_files, "Fix validation bug")
        ...
        change_type = await pr_generator._detect_change_type(docs_files, "Update documentation")
    ...
    @pytest.mark.asyncio
    async def test_generate_changelog_entry(self, pr_generator, sample_task, sample_implementation):
    ...
    @pytest.mark.asyncio
    async def test_generate_pr_with_failed_tests(self, pr_generator, sample_task, sample_implementation):
        ...
        assert "⚠️" in pr_description.test_summary or "failed" in pr_description.test_summary.lower()
