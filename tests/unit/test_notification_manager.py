...
class TestNotificationManager:
    ...
    def notification_config(self):
        ...
        return {
            "slack": {
                ...
                "channels": {"task_updates": "#dev-updates", "errors": "#alerts"},
            },
            "email": {
                "enabled": True,
    ...
    async def test_send_slack_notification_success(
        self, notification_manager, mock_plugin_registry, sample_task_event
    ):
        """Test successful Slack notification"""
        registry, slack_plugin, _ = mock_plugin_registry
        slack_plugin.send_message.return_value = {
            "success": True,
            "message_id": "msg_123",
        }
    ...
    async def test_send_slack_notification_failure(
        self, notification_manager, mock_plugin_registry
    ):
        ...
        result = await notification_manager.send_slack_notification(
            channel="#alerts", message="Error occurred", event=None
        )
    ...
    async def test_send_email_notification_success(
        ...
        email_plugin.send_email.return_value = {
            "success": True,
    ...
    @pytest.mark.asyncio
    async def test_send_task_notification_task_started(
        self, notification_manager, mock_plugin_registry, sample_task_event
    ):
        ...
        task_started_event = TaskEvent(
            type="task_started", task_id="TEST-123", agent_id="agent-001"
        )
    ...
    @pytest.mark.asyncio
    async def test_send_task_notification_task_completed(
        self, notification_manager, mock_plugin_registry, sample_task_event
    ):
    ...
    @pytest.mark.asyncio
    async def test_send_task_notification_task_error(
        self, notification_manager, mock_plugin_registry
    ):
    ...
    async def test_send_multi_channel_notification(
        ...
        results = await notification_manager.send_multi_channel_notification(
            channels=channels, message=message, event=sample_task_event
        )
    ...
    @pytest.mark.asyncio
    async def test_format_notification_message_task_completed(
        self, notification_manager, sample_task_event
    ):
        """Test notification message formatting for task completion"""
        message = notification_manager.format_notification_message(
            sample_task_event, NotificationChannel.SLACK
        )
    ...
    async def test_format_notification_message_task_error(self, notification_manager):
        ...
        message = notification_manager.format_notification_message(
            error_event, NotificationChannel.SLACK
        )
    ...
    async def test_get_notification_channel_for_event(self, notification_manager):
        ...
        channel = notification_manager.get_notification_channel(
            completed_event, NotificationChannel.SLACK
        )
        ...
        channel = notification_manager.get_notification_channel(
            other_event, NotificationChannel.SLACK
        )
    ...
    async def test_notification_retry_on_failure(
        ...
        result = await notification_manager.send_slack_notification(
            channel="#general", message="Retry test", event=sample_task_event
        )
    ...
    async def test_notification_filtering(self, notification_manager):
        ...
        low_priority_event = TaskEvent(
            type="task_started", task_id="TEST-LOW", metadata={"priority": "low"}
        )
    ...
    async def test_notification_batching(
        ...
        events = [
            TaskEvent(type="task_completed", task_id=f"TEST-{i}") for i in range(5)
        ]
