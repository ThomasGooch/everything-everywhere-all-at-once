...
class TestCodeModifier:
    ...
    @pytest.mark.asyncio
    async def test_apply_code_changes_success(self, code_modifier, temp_workspace, sample_implementation):
        """Test successful code changes application"""
        result = await code_modifier.apply_code_changes(temp_workspace, sample_implementation)
    ...
    @pytest.mark.asyncio
    async def test_apply_code_changes_with_existing_files(self, code_modifier, temp_workspace):
    ...
    @pytest.mark.asyncio
    async def test_apply_code_changes_validation_failure(self, code_modifier, temp_workspace):
    ...
    @pytest.mark.asyncio
    async def test_apply_code_changes_file_deletion(self, code_modifier, temp_workspace):
...
"""
        ...
        assert result.is_valid is False
        assert len(result.security_issues) > 0
        assert any("subprocess" in issue or "eval" in issue for issue in result.security_issues)
...
"""
    ...
    async def test_atomic_operations(self, code_modifier, temp_workspace):
        ...
        try:
            ...
            assert not cannot_create.exists(), "File should not have been created in readonly directory"
