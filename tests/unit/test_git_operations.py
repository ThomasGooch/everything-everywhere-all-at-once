...
class TestGitOperations:
    ...
    async def test_setup_repository_success(self, git_operations, temp_workspace):
        ...
        with patch("core.git_operations.git.Repo.clone_from") as mock_clone:
            ...
            result = await git_operations.setup_repository(
                temp_workspace, branch_name="feature/test-branch"
            )

            assert result is True
            mock_clone.assert_called_once_with(
                temp_workspace.repository_url, temp_workspace.path
            )
    ...
    async def test_setup_repository_clone_failure(self, git_operations, temp_workspace):
        ...
        with patch(
            "core.git_operations.git.Repo.clone_from",
    ...
    @pytest.mark.asyncio
    async def test_setup_repository_with_branch_creation(
        self, git_operations, temp_workspace
    ):
        ...
        with patch("core.git_operations.git.Repo.clone_from") as mock_clone:
            ...
            result = await git_operations.setup_repository(
                temp_workspace, branch_name="feature/new-feature", create_branch=True
            )

            assert result is True
            mock_repo_instance.create_head.assert_called_once_with(
                "feature/new-feature"
            )
    ...
    async def test_apply_file_changes_success(self, git_operations, temp_workspace):
        ...
        file_changes = {
            "test_file.py": {"content": "updated content", "action": "modify"},
            "new_file.py": {"content": "new file content", "action": "create"},
        }

        with patch(
            "core.git_operations.GitOperations._backup_existing_files"
        ) as mock_backup, patch(
            "core.git_operations.GitOperations._validate_file_changes"
        ) as mock_validate:
            mock_backup.return_value = None
            mock_validate.return_value = True

            result = await git_operations.apply_file_changes(
                temp_workspace, file_changes
            )
    ...
    async def test_apply_file_changes_with_backup(self, git_operations, temp_workspace):
        ...
        result = await git_operations.apply_file_changes(
            temp_workspace, file_changes, create_backup=True
        )
    ...
    @pytest.mark.asyncio
    async def test_apply_file_changes_validation_error(
        self, git_operations, temp_workspace
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_repository_state_success(
        self, git_operations, temp_workspace
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_repository_state_dirty(
        self, git_operations, temp_workspace
    ):
    ...
    async def test_handle_merge_conflicts(self, git_operations, temp_workspace):
        ...
        with patch("git.Repo") as mock_repo_class, patch(
            "core.git_operations.GitOperations._resolve_merge_conflicts"
        ) as mock_resolve:
            mock_repo = MagicMock()
            mock_repo_class.return_value = mock_repo
