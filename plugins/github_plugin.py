...
class GitHubPlugin(VersionControlPlugin):
    ...
    def _generate_branch_name(self, strategy: str, **kwargs) -> str:
        ...
        try:
            ...
            logger.warning(f"Missing template variable {e} for branch strategy {strategy}")
    ...
    def _get_auto_reviewers(self, file_changes: Dict[str, List[str]]) -> List[str]:
        ...
        reviewers = set()
        auto_reviewer_rules = self.config.get("options", {}).get("auto_reviewers", {}).get("rules", [])
    ...
    def _render_pr_template(self, template_type: str, template_data: Dict[str, Any]) -> str:
...
*Please review carefully and provide feedback*"""
        ...
        files_modified = "\n".join([f"- `{f}`" for f in impl_data.get("files_modified", [])])
        files_created = "\n".join([f"- `{f}`" for f in impl_data.get("files_created", [])])
        ...
        return template.format(
            ...
            acceptance_criteria=task_data.get("acceptance_criteria", "- Implementation meets requirements")
        )
...
- {task_id}"""
    ...
    async def setup_workspace_with_analysis(self, workspace_config: Dict[str, Any]) -> PluginResult:
        ...
        try:
            ...
            # Clone repository
            workspace_path = f"/tmp/workspace_{workspace_config.get('task_id', 'unknown')}"
            ...
            branch_result = await self.create_branch(actual_workspace_path, branch_name, base_branch)
    ...
    def _analyze_codebase_structure(self, workspace_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if any(workspace.rglob("*test*.py")):
                ...
                if (workspace / "pytest.ini").exists() or any("pytest" in f.name for f in workspace.rglob("*")):
    ...
    async def create_pr_with_metadata(self, pr_data: Dict[str, Any]) -> PluginResult:
        ...
        try:
            ...
            file_changes = pr_data.get("implementation_data", {}).get("file_changes", {})
            if file_changes:
                ...
                if response.status == 200:
                    ...
                    for item in tree_data.get("tree", []):
                        if item["type"] == "blob":
                            ...
                            if path in ["package.json", "package-lock.json", "yarn.lock"]:
                            ...
                            # Detect docs
                            if path.lower() in ["readme.md", "readme.txt"] or path.startswith("docs/"):
                                analysis["has_docs"] = True
        ...
        try:
            ...
            async with self._session.get(
                ...
                if response.status == 200:
                    ...
                    async with self._session.get(checks_url, headers=self._get_request_headers()) as checks_response:
                    ...
                    async with self._session.get(reviews_url, headers=self._get_request_headers()) as reviews_response:
    ...
    async def update_pr_labels_and_milestone(
        ...
        try:
            ...
            if milestone:
                ...
                async with self._session.get(milestones_url, headers=self._get_request_headers()) as ms_response:
            ...
            async with self._session.patch(
                ...
                if response.status == 200:
                    ...
                    return PluginResult(
                        success=True,
                        data={
                            "labels": [label["name"] for label in updated_data.get("labels", [])],
                            "milestone": updated_data.get("milestone", {}).get("title") if updated_data.get("milestone") else None
                        }
                    )
                else:
                    error_text = await response.text()
