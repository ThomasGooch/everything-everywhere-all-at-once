...
class ConfluencePlugin(DocumentationPlugin):
        ...
        try:
            ...
            for field in required_fields:
                if field not in self.config:
                    raise PluginValidationError(f"Missing required config field: {field}")
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            async with self._session.get(
                self._get_api_url("user/current"),
                ...
                if response.status == HTTP_OK:
                    ...
                    logger.error(f"Confluence connection test failed with status {response.status}")
                    return False
    ...
    async def get_page(self, page_id: str, expand: Optional[List[str]] = None) -> PluginResult:
        ...
        try:
            ...
            async with self._session.get(
                ...
                if response.status == HTTP_OK:
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to get page: {response.status} - {error_text}")
                    return PluginResult(
                        success=False,
    ...
    async def search_pages(
        ...
        try:
            ...
            async with self._session.get(
                self._get_api_url(f"content/search?cql={cql_query}&limit={limit}&expand=excerpt"),
                headers=self._get_request_headers()
            ) as response:
                ...
                if response.status == HTTP_OK:
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to search pages: {response.status} - {error_text}")
                    return PluginResult(
                        success=False,
    ...
    async def create_page_from_template(self, template_data: Dict[str, Any]) -> PluginResult:
    ...
    def _render_template(self, template_type: str, variables: Dict[str, Any]) -> str:
        ...
        template = templates.get(template_type, "<h1>{{title}}</h1><p>Template not found</p>")
        ...
        try:
            ...
            async with self._session.post(
                ...
                if response.status == HTTP_OK:
                    ...
                    added_labels = [result["name"] for result in response_data.get("results", [])]
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to add labels: {response.status} - {error_text}")
                    return PluginResult(
                        success=False,
    ...
    async def get_page_attachments(self, page_id: str) -> PluginResult:
        ...
        try:
            async with self._session.get(
                self._get_api_url(f"content/{page_id}/child/attachment?expand=download"),
                headers=self._get_request_headers()
            ) as response:
                ...
                if response.status == HTTP_OK:
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to get attachments: {response.status} - {error_text}")
                    return PluginResult(
                        success=False,
    ...
    async def upload_attachment(self, page_id: str, file_path: str, filename: str) -> PluginResult:
        ...
        try:
            ...
            with open(file_path_obj, 'rb') as file:
                ...
                async with self._session.post(
                    ...
                    headers={"Authorization": f"Basic {self._get_auth_token()}"},  # Skip content-type for multipart
                    data=data
                ) as response:
    ...
    async def get_page_history(self, page_id: str, limit: int = 10) -> PluginResult:
        ...
        try:
            async with self._session.get(
                self._get_api_url(f"content/{page_id}/history?limit={limit}&expand=by,when"),
                headers=self._get_request_headers()
            ) as response:
                ...
                if response.status == HTTP_OK:
                    ...
                    error_text = await response.text()
                    logger.error(f"Failed to get page history: {response.status} - {error_text}")
                    return PluginResult(
                        success=False,
