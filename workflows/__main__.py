    ...
    async def execute_workflow(
        ...
        try:
            ...
            if result.success:
                ...
                print(f"📊 Steps completed: {len([s for s in result.step_results if s.success])}/{len(result.step_results)}")
...
Examples:
  ...
  python -m workflows validate --workflow standard_dev_workflow
    ...
    execute_parser.add_argument(
        '--workflow', 
        required=True,
        help='Workflow name (without .yaml extension)'
    )
    execute_parser.add_argument(
        '--task-id',
        help='Task ID for task management workflows'
    )
    execute_parser.add_argument(
        '--repository-url', 
        help='Repository URL for version control workflows'
    )
    execute_parser.add_argument(
        '--base-branch',
        default='main',
    ...
    execute_parser.add_argument(
        '--notify-team',
        action='store_true',
        help='Enable team notifications'
    )
    execute_parser.add_argument(
        '--team-channel',
        default='#development',
        help='Team channel for notifications'
    )
    execute_parser.add_argument(
        '--agent-id',
        default='ai-dev-agent-001',
        help='Agent identifier for tracking'
    )
